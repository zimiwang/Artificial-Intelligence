...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for action in legal:
            successor_position = (pacmanPosition[0] + Actions.directionToVector(action)[0],
                                  pacmanPosition[1] + Actions.directionToVector(action)[1])
            min_ghost_distance = min(self.distancer.getDistance(successor_position, ghost) for ghost in maxBelief)

            if min_ghost_distance < min_distance:
                best_action = action
                min_distance = min_ghost_distance
